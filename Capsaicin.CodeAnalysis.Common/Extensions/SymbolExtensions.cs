...
public static class SymbolExtensions
{
    ...
    public static bool HasAttribute(this ISymbol symbol, ITypeSymbol attributeClass, bool includeSpecializing = false)
    {
        return includeSpecializing 
            ? symbol.GetAttributes().Any(a => a.EqualsOrIsSpecializingAttributeClass(attributeClass))
            : symbol.GetAttributes().Any(a => a.EqualsAttributeClass(attributeClass));
    }
    ...
    /// <param name="includeSpecializing"></param>
    /// <returns></returns>
    public static IEnumerable<AttributeData> GetAttributes(this ISymbol symbol, ITypeSymbol attributeClass, bool includeSpecializing = false)
    {
        return includeSpecializing
            ? symbol.GetAttributes().Where(a => a.EqualsOrIsSpecializingAttributeClass(attributeClass))
            : symbol.GetAttributes().Where(a => a.EqualsAttributeClass(attributeClass));
    }
}
